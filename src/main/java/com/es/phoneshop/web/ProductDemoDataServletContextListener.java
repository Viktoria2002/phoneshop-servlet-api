    ...
    public List<PriceHistory> getSampleProductsPriceHistory() {
        ...
        for (Product product : products) {
            for (int i = 0; i < 2; i++) {
                ...
                histories.add(new PriceHistory(startDate.minusMonths(1 + (int) (Math.random() * 15)),
                        new BigDecimal(100 + (int) (Math.random() * 400)), usd, product));
            }
            histories.add(new PriceHistory(startDate.minusMonths(1 + (int) (Math.random() * 15)),
                    product.getPrice(), usd, product));
        }
        return histories;
    }
}
